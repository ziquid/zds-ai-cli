...
- [✅] 10: Fixed var: output showing "Values (0)" - now shows "No direct values (renders from children/getters)"
...
- [✅] 13: Fixed orphaned grandchildren not appearing in parent render - automatically creates intermediate parents (e.g., MESSAGE:ALL when MESSAGE:ALL:CURRENT exists)
...
- [✅] 22: Fixed all 10 Dependabot security vulnerabilities blocking v0.1.9 release (Issue #59)
  - Update `@modelcontextprotocol/sdk` to 1.26.0 (fixes ReDoS and cross-client data leak)
  - Update `ink` to 6.6.0 (fixes lodash prototype pollution)
  - Update `axios` to 1.13.4
  - Add `qs` override to 6.14.1 (fixes DoS vulnerability)
  - Update dev dependencies (@types/node, @typescript-eslint/*)
  - Verified with `bun audit`: No vulnerabilities found ✅
